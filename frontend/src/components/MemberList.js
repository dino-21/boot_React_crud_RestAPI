  ...
  const handleDelete = (id) => {
    if (window.confirm("정말 삭제하시겠습니까?")) {
      axios.delete(`http://localhost:8080/api/members/${id}`)
        .then(() => {
          alert("삭제 완료!");
          fetchPage(currentPage);  //  올바른 함수 이름으로 수정
        })
        .catch(err => {
  ...
  return (
    <div className="container mt-4">
      ...
      <Table striped bordered hover responsive className="text-center">
        <thead>
          <tr>
            <th>ID</th><th>NAME</th><th>AGE</th><th>PHONE</th><th>ADDRESS</th><th>ACTIONS</th>
          </tr>
        </thead>
        <tbody>
          {members.map(member => (
            <tr key={member.id}>
              ...
              <td>
                <Link to={`/edit/${member.id}`}>
                  <Button size="sm" variant="warning" className="me-2">수정</Button>
                </Link>
                <Button variant="danger" size="sm" onClick={() => handleDelete(member.id)}>삭제</Button>
